...
var Medecine = _db["default"].medecine;
var Op = _db["default"].Sequelize.Op; // Create and Save a new Medecine
      ...
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            ...
            tutorial = req.body;
            _context.next = 5;
            return Medecine.create(tutorial);
          ...
          case 9:
            ...
            res.status(500).send({
              message: err.message || 'Some error occurred while creating the Medecine.'
            });
...
}(); // Retrieve all Medecines from the database.
...
}(); // Find a single Medecine with an id
      ...
      while (1) {
        switch (_context8.prev = _context8.next) {
          ...
          case 8:
            ...
            res.status(500).send({
              message: 'Error retrieving Medecine with id=' + id
            });
...
}(); // Update a Medecine by the id in the request
      ...
      while (1) {
        switch (_context9.prev = _context9.next) {
          case 0:
            ...
            _id2 = req.params.id;
            _context9.next = 4;
            return Medecine.update(req.body, {
              where: {
                id: _id2
              }
            });
...
}(); // Delete a Medecine with the specified id in the request
...
var Delete = function Delete(req, res) {
  ...
  Medecine.destroy({
    ...
    if (num == 1) {
      res.send({
        message: 'Medecine was deleted successfully!'
      });
    } else {
      res.send({
        message: "Cannot delete Medecine with id=".concat(id, ". Maybe Medecine was not found!")
      });
    }
  })["catch"](function (err) {
    res.status(500).send({
      message: 'Could not delete Medecine with id=' + id
    });
  });
}; // Delete all Medecines from the database.
...
var deleteAll = function deleteAll(req, res) {
  Medecine.destroy({
    ...
    res.send({
      message: "".concat(nums, " Medecines were deleted successfully!")
    });
  })["catch"](function (err) {
    res.status(500).send({
